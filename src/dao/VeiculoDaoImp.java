...
public class VeiculoDaoImp implements VeiculoDao {
	...
	public void adicionar(Veiculo v) throws ClassNotFoundException, SQLException {
		...
		sb.append("INSERT INTO tb_veiculo (placa, marca, modelo, ano_fab, ano_mod, chassi, status)");
		sb.append("VALUES (?, ?, ?, ?, ?, ?, ?)");
		...
		st.setString(6, v.getChassi());
		st.setString(7, EnumStatusVeiculo.DISPONIVEL.toString());
	...
	public void alterar(Veiculo v) throws ClassNotFoundException, SQLException {
		{
			...
			StringBuffer sb = new StringBuffer();
			sb.append("UPDATE tb_veiculo SET marca = ?, modelo = ?, ano_fab = ?, ano_mod = ?, chassi = ?, status = ? ");
	...
	public Veiculo buscarPorPlaca(String placa) throws ClassNotFoundException, SQLException {
		...
		Connection con = DatabaseConnection.getConnection();
		StringBuffer sb = new StringBuffer();
		sb.append("SELECT placa, marca, modelo, ano_fab, ano_mod, chassi, ");
